...
class Driver {
  ...
  class Rider {
    constructor(riderId, xCoordinate, yCoordinate) {
  ...
  class RideManager {
    ...
    match(riderId) {
      ...
      const availableDrivers = Object.values(this.drivers)
      ...
      for(let i=0; i<distances.length;i++){
          if (parseInt(distances[i].distance) >= 1 && parseInt(distances[i].distance) <= 5) {
    ...
    startRide(rideId, n, riderId) {
      ...
      if (!matchedDrivers || matchedDrivers.length === 0 || n > matchedDrivers.length) {
        console.log('INVALID_RIDE');
        return;
      }
    ...
    stopRide(rideId, destinationX, destinationY, timeTaken) {
      ...
      let obj ={}
      obj[rideId]= `BILL ${rideId} ${driverId} ${totalAmount.toFixed(2)}`
